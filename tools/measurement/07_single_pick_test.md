...
회전 시험: `--rotate --reference-yaw-deg 실측값`을 추가한다. 처음에는 반드시
`--hover-only`로 방향/XY만 확인한다. 이 모드는 그리퍼와 집기 Z 하강을 실행하지 않는다.
기준각 측정과 실행 명령은 [박스 회전 시험 안내](box_orientation.md)를 참고한다.
옵션을 생략하면 기존 고정 방향 시험이다.
